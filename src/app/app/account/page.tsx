...
import { formatStaffRole, loadStaffWorkspace, type StaffWorkspace } from "@/lib/workspace";
...
export default function StaffAccountPage() {
  ...
  useEffect(() => {
    ...
    async function loadAccount() {
      ...
      setFirstName(result.workspace.profile.first_name);
      setLastName(result.workspace.profile.last_name);
    }
  ...
  async function deleteAccount(event: FormEvent<HTMLFormElement>) {
    ...
    if (workspace.isPlatformAdmin) {
      setDeleteMessage("Transfer platform administration before deleting this account.");
      return;
    }
    ...
    <AppShell>
      <div className="space-y-6">
        <section className="premium-card">
          ...
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Manage your firm account</h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
            Update your profile, secure your password, and manage your individual CivicFlow login.
          </p>
        </section>
        ...
        <section className="grid items-start gap-6 xl:grid-cols-2">
          <form onSubmit={saveProfile} noValidate className="premium-card">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <label className="input-label">First name *<input value={firstName} onChange={(event) => { setFirstName(event.target.value); setProfileMessage(""); }} className="input-field" required /></label>
              <label className="input-label">Last name *<input value={lastName} onChange={(event) => { setLastName(event.target.value); setProfileMessage(""); }} className="input-field" required /></label>
            </div>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-900">{workspace ? formatStaffRole(workspace.profile.role) : "Loading..."}</p>
              </div>
            </div>
        ...
        <form onSubmit={deleteAccount} noValidate className="premium-card border-rose-200">
          ...
          <h2 className="mt-3 text-xl font-bold text-slate-950">Delete your individual CivicFlow login</h2>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
            Deleting your user account does not delete the law firm's organization, client matters, or records. Sole firm administrators must transfer administration first.
          </p>

          {workspace?.isPlatformAdmin ? (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium leading-6 text-amber-800">
              This account is the CivicFlow platform administrator. Self-service deletion is disabled until platform administration is transferred.
            </div>
          ) : (
            <>
