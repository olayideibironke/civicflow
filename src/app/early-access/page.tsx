...
export default function EarlyAccessPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validationError = getFirstValidationError([
      ...
      validateRequiredText(formState.primaryReason, "Primary reason"),
    ]);
    ...
    const { error } = await supabase.from("demo_requests").insert({
      ...
      role_title: "Early access participant",
  ...
  return (
    <main className="min-h-screen text-slate-900">
      <MarketingHeader activePage="early-access" />
      ...
      <section className="mx-auto grid max-w-[1200px] gap-8 px-6 py-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-start lg:py-16">
        <aside className="premium-dark animate-fade-up lg:sticky lg:top-28 lg:!p-9">
          ...
          <h1 className="mt-7 text-3xl font-bold leading-[1.1] tracking-tight text-white xl:text-4xl">
            Help us validate a simpler law-practice platform.
          </h1>
          <p className="mt-5 text-base leading-7 text-slate-300">
            We are testing demand for a full small-firm operating platform planned from $69 per attorney per month, with fewer paid feature gates and a focus on simplicity.
          </p>
          ...
          <div className="mt-7 grid gap-2.5">
            {[
              ...
              "Planned pricing is not a final commercial commitment.",
              "The product is currently in validation and development.",
            ].map((item) => (
        ...
        <section className="animate-fade-up rounded-2xl border border-slate-200/80 bg-white/95 shadow-[var(--shadow-md)] backdrop-blur">
          {submitted ? (
            <div className="p-8">
              ...
              <p className="eyebrow mt-6 text-emerald-600">You are on the list</p>
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Early access interest received.</h2>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                Thank you. Your response will be used to measure whether this product direction and pricing are worth taking into full development.
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/pricing" className="btn btn-primary">Review pricing concept</Link>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div className="border-b border-slate-100 p-8">
                <p className="eyebrow text-blue-600">Market validation</p>
                <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Join early access.</h2>
                <p className="mt-2.5 text-base leading-7 text-slate-600">
                  We only need enough information to understand the type of firm showing interest and what is driving the search for another platform.
                </p>
              </div>
              ...
              <div className="p-8">
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <label className="input-label md:col-span-2">
                    Main reason you would consider another platform *
                    <select required value={formState.primaryReason} onChange={(event) => updateField("primaryReason", event.target.value)} className="input-field">
                      <option>Price</option>
                      <option>Missing features</option>
                      <option>Too complicated</option>
                      <option>Mobile experience</option>
                      <option>Documents</option>
                      <option>Billing or accounting</option>
                      <option>Reporting</option>
                      <option>Integrations or API</option>
                      <option>Client communication</option>
                      <option>Migration concerns</option>
                      <option>Other</option>
                    </select>
                  </label>
                </div>
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
                  <p className="max-w-xl text-sm leading-6 text-slate-500">
                    Your response is an expression of interest only. It is not a purchase, contract, or reservation.
                  </p>
